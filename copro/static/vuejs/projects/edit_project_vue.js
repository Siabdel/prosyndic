...
var compo_member = {
  ...
  computed:{
    ...
    SortedMembersDispo() {
      ...
      if (this.users_dispo.length > 0) {
        console.log( "User dispos = ", this.users_dispo[0]  )
        console.log( "SortedMembersDispo filterBy = ", this.sortBy)
        return this.users_dispo.filter( member  => member.full_name.includes(this.filterBy))
  ...
  // fonction check : Selection des membres
   ...
   members_disponible(){
    ...
    api_delete_project(){
      //--
      var url = `/pro/api/project/del/${project_id}.json`

      fetch(url)
        .then(response =>  response )
        .then( result    => {
          this.users_dispo = result.json();
          console.log( "api_delete_project = ", project_id );
        })
        .catch((err)=>console.log(err))
    },
    ...
    methods:{
      ...
      api_members_project(event, project_id){
        ...
        .then( result => {
          ...
          console.log("api_members_project ... = " + this.members_project )
        })
        .catch((err)=>console.log(err))
      ...
      // -- API UPLOAD FILE avec feetch
      api_upload_file(project_id){
        //
        var active = false

        var url = `/pro/api/upload_doc/`
        var form = $("#form_01").serialize()
        var cle_csrf = $( "input[name='csrfmiddlewaretoken']" ).val()
        var model_name = $( "input[name='model_name']" ).val()
        var model_id = $( "input[name='model_id']" ).val()

        var titre = $( "input[id='id_do_title']" ).val()
        var description = $( "textarea[id='id_do_description']" ).val()

        console.log("api_upload_file mo model =  " + model_name)
        ...
        formdata.append('do_title', titre)
        formdata.append('active', active)
        formdata.append('do_description', description)
        formdata.append('csrfmiddlewaretoken', cle_csrf)
        ...
        fetch(url,
          ...
          //location.href = "/da/cac/list/"
          var url_details = `/pro/projects/${project_id}/`
          location.replace(url_details)
   				return  response
   		 }).catch((err)=>console.error(err))

       }, // fin upload_file,
      // save formulaire ticket
      save_form_ticket(confirmer){

        $("#condirm_maj").modal('show')

        if (confirmer) {
          console.log("sauvegarde ticket confirmer ...")
          $("#id_formulaire_ticket").submit()
          $("#quit_02").click()

        }else{
          console.log("sauvegarde ticket non confirmer !!")
        }
      },

      // save formulaire projet
      save_form_project(confirmer){

        $("#condirm_maj").modal('show')

        if (confirmer) {
          $('#id_formulaire_project').submit()
          $("#quit_02").click()
          console.log("sauvegarde project confirmer ...")

        }else{
          console.log("sauvegarde project non confirmer !!")
        }
      }
